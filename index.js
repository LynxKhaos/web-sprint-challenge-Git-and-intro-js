...
if(artists[i].years.split('-')[0] >= 1900 && artists[i].years.split('-')[1] <= 2000){
artists1900s.push(artists[i].name)
    }
  }
  return artists1900s
}
...
const artistAlex = addArtist(artists, 'Alex', '1997 - 2022', 'abstract', 'American', 'lorem ipsum')
...
function lotsOfArt(artists) {
  ...
  if(artists[i].paintings > 100){
    listOfArtists.push(artists[i].name)
   }
...
function artistByCountry(artists, nationality){
  let artistByCountryArr = []
  for(let i = 0; i <= artists.length - 1; i++){
    if(artists[i].nationality.split(1).includes(nationality)){
      artistByCountryArr.push(artists[i].name)
    }
  }
  return artistByCountryArr
  }
